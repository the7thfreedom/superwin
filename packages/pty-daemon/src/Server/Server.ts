 ...
 * filesystem entry. Detect them so we can skip `mkdir`/`unlink`/`chmod`.
 */
...
export class Server {
	...
	async listen(): Promise<void> {
		const isPipe = isNamedPipePath(this.opts.socketPath);
		if (!isPipe) {
		...
		// Owner-only access. The socket file IS the auth boundary on POSIX.
		// Windows named pipes have no filesystem entry — owner-only ACL is
		// applied automatically by the kernel when no SECURITY_ATTRIBUTES
		// are supplied. Node's net module doesn't expose pipe ACLs, so we
	...
	async close(opts: { killSessions?: boolean } = {}): Promise<void> {
		...
		if (!isNamedPipePath(this.opts.socketPath)) {
			try {
				fs.unlinkSync(this.opts.socketPath);
			} catch {
				// ignore
