...
/**
 * Windows named pipes are addressed as `\\.\pipe\<name>` and have no
 * filesystem entry. Detect them so we can skip `existsSync`/`unlink`/etc.
 */
function isNamedPipePath(p: string): boolean {
 ...
 *
 * On Windows there is no UDS support in practice (Node attempts to bind a
 * regular file path → EACCES), so we use a named pipe instead. Pipes live
 * in the kernel object namespace, not on disk, so the same uniqueness
 * argument (hash of org id) applies.
 */
...
async function waitForSocket(
	...
	const isPipe = isNamedPipePath(socketPath);
	while (Date.now() < deadline) {
		// Named pipes have no filesystem entry — skip `existsSync` and
		// rely on connect probes alone.
		if (isPipe || fs.existsSync(socketPath)) {
