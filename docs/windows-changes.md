...
### 2026-06-10 — Fresh-clone setup and installer-build fixes

| Area | Path(s) | What changed |
|------|---------|--------------|
| Env template | `.env.example`, `.gitignore` | Restored `.env.example` byte-identical to upstream `main` — the README's first setup step copies it, but the `.env.*` ignore rule silently dropped it during the snapshot import. Added a `!.env.example` negation so it cannot drop out again. Note: the template header references `./.superset/setup.local.sh` and `.env.local.example`, which exist upstream but are not yet imported into this fork. |
| Lockfile | `bun.lock` | Added the missing `native-keymap@3.3.9` entry (declared in `package.json` since the dependency was re-added, but never committed to the lock), which made `bun install --frozen-lockfile` fail on every fresh clone. |
| Native modules | `scripts/copy-native-modules.ts` | Under Bun's isolated linker (`bunfig.toml`), `@duckdb/*` is a transitive dep (via `@mastra/duckdb`) and never gets a workspace-root copy, so `copyDuckdbPlatformPackages` hit its missing-package early-return and `validate:native-runtime` later failed with a missing `@duckdb/node-bindings-win32-x64`. Now materializes `@duckdb/node-api` and `@duckdb/node-bindings` from the Bun store flat index first (same mechanism `copyModuleIfSymlink` already uses), healing torn partial copies (directory without `package.json`) before retrying, then prepares the platform binding as before. Verified from a clean `node_modules/@duckdb`: all three packages materialize, `validate:native-runtime` passes, and the packaged asar contains `@duckdb/*` with `duckdb.node` smart-unpacked. |
| CI | `.github/workflows/{ci,build-desktop-windows,triage-issue}.yml` | All seven install steps used `bun install --frozen --ignore-scripts`, but `--frozen` is not a bun flag (only `--frozen-lockfile` is) — it is silently ignored, bun re-resolves and rewrites the lockfile in the runner, and drift passes green. This is how the `native-keymap` lockfile drift reached `main`. Switched to `--frozen-lockfile` so package.json/lockfile drift fails CI. |
| Docs | `README.md`, `AGENTS.md` | Fixed the build command in both docs (`bun run build:win` never existed → `bun run build`, NSIS output in `release/`; AGENTS.md also still instructed adding a `build:win` script under the old `apps/desktop/` layout). Documented Windows build prerequisites: VS 2022 "Desktop development with C++" workload (MSVC Spectre-mitigated libs additionally needed for `node-pty`), and Windows Developer Mode so electron-builder can extract its symlink-containing winCodeSign cache (elevation works too but recurs on electron-builder upgrades). |
