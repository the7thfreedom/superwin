...
//
// On POSIX we use a Unix Domain Socket inside `$SUPERSET_HOME_DIR`. On
// Windows we use a named pipe — pipes have no filesystem entry, so any
// `existsSync` / `unlinkSync` / `chmodSync` call against the path must be
// gated behind `IS_PIPE`.
const IS_PIPE = process.platform === "win32";
const SOCKET_PATH = IS_PIPE
...
function isSocketLive(): Promise<boolean> {
	return new Promise((resolve) => {
		// Named pipes have no filesystem entry; skip the cheap existsSync
		// guard and rely on the connect probe to decide.
		if (!IS_PIPE && !existsSync(SOCKET_PATH)) {
			resolve(false);
			return;
		}
...
async function startServer(): Promise<void> {
	...
	if (IS_PIPE || existsSync(SOCKET_PATH)) {
		...
		// Named pipes vanish when their server closes, so nothing to unlink.
		if (!IS_PIPE) {
			try {
	...
	await new Promise<void>((resolve, reject) => {
		...
		newServer.listen(SOCKET_PATH, () => {
			...
			// Named pipes have no filesystem entry — kernel defaults apply.
			if (!IS_PIPE) {
				try {
...
async function stopServer(): Promise<void> {
	...
	try {
		if (!IS_PIPE && existsSync(SOCKET_PATH)) unlinkSync(SOCKET_PATH);
