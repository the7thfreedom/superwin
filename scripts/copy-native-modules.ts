...
function copyModuleIfSymlink(
	...
	if (stats.isSymbolicLink()) {
		...
		// Remove the symlink. On Windows, directory symlinks cannot be removed
		// with a plain `rmSync(path)` — Bun reports `EFAULT` on Windows because
		// the entry is reparse-pointed; we need either `unlinkSync` (POSIX) or
		// `rmSync(path, { recursive: true, force: true })` (Windows handles the
		// reparse point correctly when explicit recursion is allowed even though
		// the symlink target isn't traversed).
