...
const config: Configuration = {
	...
	// The `install:deps` script in the prebuild chain already runs
	// `electron-builder install-app-deps`, and re-running here picks up the
	// materialized (no-longer-symlinked) copies that lack `node-addon-api` at
	// their resolution root, which fails on Windows with Bun isolated installs.
	npmRebuild: false,
